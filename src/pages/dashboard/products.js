...
export default function Products() {
  ...
  useEffect(() => {
    async function getProducts() {
      const response = await axios.get(endPoints.products.product);
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {products?.map((product) => (
                    <tr key={`Product-item-${product.id}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap"></td> <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">${product.id}</span>
